...
describe('controller persistence', () => {
  it('round-trips machine/inbox/feedback/status', () => {
    const store = new RunStore(ws, 'r1')
    const s = { machine: initMachine(plan), inbox: [{ id: 'g1', kind: 'gate', stageKey: 'design', body: 'b' }], feedback: [{ id: 'f1', text: 'note' }], outcomes: {}, status: 'awaiting' as const }
