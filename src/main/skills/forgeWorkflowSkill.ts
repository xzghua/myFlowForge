...
2. **按需裁剪范围（重要，省 token）** —— 不要一律跑完整工作流的所有阶段、改动所有项目。**根据任务大小和涉及范围，只选必要的阶段与项目**：
   - 小需求 / 局部改动：用 \`stages\` 只传需要的阶段 key（如只做 \`["requirement","develop"]\`，跳过测试与 CR）；只涉及某几个项目时用 \`projects\` 只传相关项目名，不要动无关项目。
   - 大需求 / 需要严格保障：再跑完整阶段与全部相关项目。
   - 不确定要不要精简时，可在方案要点里说明你打算跑哪些阶段/项目，让用户在审批时确认。
